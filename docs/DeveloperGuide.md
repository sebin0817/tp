...
* `LogicManager#addCommand(UndoableCommand )`— Adds an `UndoableCommand` to the history stack *(more of deque data
...
archive file. The format for the archived file's name is addressBook_<currentDateTime>.json, where <currentDateTime> is
...
3. Rename the copied file to addressBook_<currentDateTime>.json, accurately reflecting the date and time of the
   archiving operation.
